...
final staffAssignmentsRepositoryProvider =
    Provider<StaffAssignmentsRepository>((ref) {
  final dio = ref.watch(apiClientProvider);
  return StaffAssignmentsRepository(StaffAssignmentsApi(dio));
});

final myStaffAssignmentsProvider =
    FutureProvider<List<StaffAssignment>>((ref) async {
...
final staffAssignmentHistoryProvider =
    FutureProvider.family<List<StaffAssignmentHistoryEntry>, HistoryQuery>(
  (ref, query) async {
    final repository = ref.watch(staffAssignmentsRepositoryProvider);
    return repository.getHistory(
        eventId: query.eventId, assignmentId: query.assignmentId);
  },
);
